...
def update_cuestionario(id_cuestionario: int,cuestionario: cuestionarios_schema.CuestionarioCreate,
    db: Session = Depends(get_db)
):
    # 1. Validar que el cuestionario exista
    db_cuestionario = cuestionarios_crud.get_cuestionario_by_id(db, id_cuestionario=id_cuestionario)
    if db_cuestionario is None:
        raise HTTPException(status_code=404, detail="Cuestionario no encontrado")
    ...
    # 3. Validar que no se repita el nombre para ese usuario
    db_cuestionario_existente = (
        db.query(Cuestionario)
        .filter(
            Cuestionario.nombre == cuestionario.nombre,
            Cuestionario.id_usuario == cuestionario.id_usuario,
            Cuestionario.id_cuestionario != id_cuestionario  # 👈 excluye el mismo cuestionario
        )
        .first()
    )
    if db_cuestionario_existente:
        raise HTTPException(status_code=400, detail="Ese cuestionario ya existe para este usuario")

    # 4. Actualizar cuestionario
    return cuestionarios_crud.update_cuestionario(
        db=db,
