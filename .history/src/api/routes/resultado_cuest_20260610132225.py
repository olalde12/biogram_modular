...
@router.delete('/delete/{id_rescues}', response_model=resultado_schema.ResultadoCuest)
def delete_resultado_cuestionario(id_resultado: int, db: Session = Depends(get_db)):
    db_resultado = resultado_crud.get_rescues_by_id(db, id_resultado_cues=id_)
