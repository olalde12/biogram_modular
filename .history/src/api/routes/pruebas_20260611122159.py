...
from src.models import Pruebas_bioquimicas as Prueba
...
def read_pruebas(db: Session = Depends(get_db)):
    db_pruebas = pruebas_crud.get_pruebas(db)
    return db_pruebas
...
def create_prueba(prueba: pruebas_schema.PruebaCreate, db: Session = Depends(get_db)):
    db_prueba = Prueba(
        ...
        fundamento=prueba.fun
    )
    pruebas_crud.get_prueba_by_name(db, nombre=prueba.nombre)
    if db_prueba:
