...
def create_usuario(usuario: usuarios_schema.UsuarioCreate, db: Session = Depends(get_db)):
    ...
    if not re.match(regex2, usuario.nombre) or not re.match(regex2, usuario.apellido):
        raise HTTPException(status_code=400, detail="EL nombre y el apellido solo debe contener letras")
