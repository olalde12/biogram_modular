...
def create_resultado(resultado: resultado_schema.ResultadoCuestCreate, db: Session = Depends(get_db)):
    ...
    db_resultado_existente = (db.query(Resultado_cuestionario)
        .filter(
            ResultadoCuestionario.id_cuesti == resultado.id_cuesti,
            ResultadoCuestionario.id_prueba == resultado.id_prueba
        )
        .first()
    )
    if db_resultado_existente:
        raise HTTPException(status_code=400, detail="Esa prueba ya está registrada en este cuestionario")
...
def update_resultado(id_resultado: int, resultado: resultado_schema.ResultadoCuestCreate, db: Session = Depends(get_db)):
    db_resultado = resultado_crud.get_rescues_by_id(db, id_resultado_cues=id_resultado)
    if db_resultado is None:
        raise HTTPException(status_code=404, detail="No existe tal resulatdo")
    db_cuestionario = cuestionario_crud.get_usuario_by_id(db, id_cuestionario=resultado.id_cuesti)
    if not db_cuestionario:
