...
def update_comentario(id_comentario: int, id_usuario: int, id_publicacion: int, comentario: comentario_schema.ComentarioUpdate, db: Session = Depends(get_db)):
    db_comentario = comentario_crud.get_comentario_by_id(db, id_comentario=id_comentario)
    if db_comentario is None:
    ...
    if not db_publicacion:
        raise HTTPException(status_code=404, detail="Esa publicacion no existe")
    if db_comentario.id_publicacion != id_publicacion:
        raise HTTPException(status_code=404, detail="Ese comentario no pertenece a esa publicacion")
    if db_comentario.id_usuario != id_usuario:
        raise HTTPException(status_code=404, detail="Ese comentario no le pertenece a ese usuario")
    return comentario_crud.update_comentario(db=db, id_comentario=id_comentario, comentario=comentario)

# Ruta para eliminar un comentario
@router.delete('/delete/{id_comentario}', response_model=comentario_schema.Comentario)
def delete_comentario(id_comentario: int, id_usuario: int, db: Session = Depends(get_db)):
    db_usuario = usuario_crud.get_usuario_by_id(db, id_usuario=id_usuario)
    if not db_usuario or db_usuario.estado :
        raise HTTPException(status_code=404, detail='Ese usuario no exite')
    db_comentario = (db.query(Comentarios).filter(
        Comentarios.id_comentario == id_comentario,
        Comentarios.id_usuario == id_usuario
    ).first())
    if db_comentario is None:
        raise HTTPException(status_code=404, detail='Comentario no encontrado')
    return comentario_crud.delete_comentario(db=db, id_comentario=id_comentario)
