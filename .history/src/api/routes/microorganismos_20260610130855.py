...
def agregar_medio(id_micro: int, id_medio: int, db: Session = Depends(get_db)):
    ...
    if not micro:
        raise HTTPException(status_code=404, detail="Microorganismo no encontrado")
    medio = db.query(Medio_cultivo).filter(Medio_cultivo.id_medio == id_medio).first()
    if not medio:
        raise HTTPException(status_code=404, detail="Medio de cultivo no encontrado")
    relacion = db.query(medio_microorganismo).filter(
        medio_microorganismo.id_micro == id_micro,
        medio_microorganismo.id_medio == id_medio
    ).first()
    if relacion:
        raise HTTPException(status_code=400, detail="Ya existe esa relación")
    nueva_relacion = medio_microorganismo(id_micro=id_micro, id_medio=id_medio)
    db.add(nueva_relacion)
    db.commit()
    return {"msg": "Relación creada correctamente"}

# Ruta para obtener microorganismo y medio
@router.get("/microorganismo/{id_micro}/medios")
def get_medios_de_microorganismo(id_micro: int, db: Session = Depends(get_db)):
    micro = db.query(Microorganismo).filter(Microorganismo.id_microorganismo == id_micro).first()
    if not micro:
        raise HTTPException(status_code=404, detail="Microorganismo no encontrado")
    relaciones = db.query(medio_microorganismo).filter(medio_microorganismo.id_micro == id_micro).all()
    return relaciones

# Ruta para eliminar la relacion (medio y microorganismo)
@router.delete("/microorganismo/{id_micro}/medios/{id_medio}")
def delete_medio_microorganismo(id_micro: int, id_medio: int, db: Session = Depends(get_db)):
    relacion = db.query(medio_microorganismo).filter(
        medio_microorganismo.id_micro == id_micro,
        edioMicroorganismo.id_medio == id_medio
    ).first()
    if not relacion:
    ...
    db.delete(relacion)
    db.commit()
    return {"msg": "Relación eliminada correctamente"}
