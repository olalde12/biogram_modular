...
def create_comentario(comentario: comentario_schema.ComentarioCreate, db: Session = Depends(get_db)):
    ...
    db_comentario = (db.query(Comentarios).filter(
        Comentarios.contenido == comentario.contenido,
        Comentarios.id_publicacion == comentario.id_publicacion,
        Comentarios.id_usuario == comentario.id_usuario,
    ).first())
    return comentario_crud.create_publicacion(db=db, publicacion=publicacion)
