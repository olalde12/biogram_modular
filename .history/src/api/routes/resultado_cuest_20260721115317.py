...
def update_resultado(id_resultado: int, id_cuestionario: int, id_prueba: int, resultado: resultado_schema.ResultadoCuestCreate, db: Session = Depends(get_db)):
    db_resultado = resultado_crud.get_rescues_by_id(db, id_resultado_cues=id_resultado)
    if db_resultado is None:
        raise HTTPException(status_code=404, detail="No existe tal resultado")
    db_cuestionario = cuestionario_crud.get_cuestionario_by_id(db, id_cuestionario=id_cuesti)
    if not db_cuestionario:
