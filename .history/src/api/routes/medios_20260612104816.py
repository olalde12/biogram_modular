from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.orm import Session
from typing import List
from src.api.deps import get_db
from src.crud import medios as medios_crud
from src.schemas import medio_cultivo as medios_schema

router = APIRouter()

# Ruta para obtener todas los medios
@router.get('/', response_model=List[medios_schema.Medio])
def read_medio(db: Session = Depends(get_db)):
    db_medio = medios_crud.get_medios(db)
    return db_medio

# Ruta para obtener un medio por id
@router.get('/{id_medio}', response_model=medios_schema.Medio)
def read_medio_by_id(id_medio: int, db: Session = Depends(get_db)):
    db_medio = medios_crud.get_medio_by_id(db, id_medio=id_medio)
    if db_medio is None:
        raise HTTPException(status_code=404, detail='Medio no encontrado')
    return db_medio

# Ruta para obtener un medio por nombre
@router.get('/name/{nombre}', response_model=medios_schema.Medio)
def read_medio_by_name(nombre: str, db: Session = Depends(get_db)):
    db_medio = medios_crud.get_medio_by_name(db, nombre=nombre)
    if db_medio is None:
        raise HTTPException(status_code=404, detail='Medio no encontrado')
    return db_medio

# Ruta para crear un medio
@router.post('/create', response_model=medios_schema.Medio)
def create_medio(medio: medios_schema.MedioCreate, db: Session = Depends(get_db)):
    db_medio = medios_crud.get_medio_by_name(db, nombre=medio.nombre)
    if db_medio:
        raise HTTPException(status_code=400, detail='Esa medio ya existe')
    return medios_crud.create_medios(db=db, medio=medio)

# Ruta para modificar un medio
@router.put('/update/{id_medio}', response_model=medios_schema.Medio)
def update_medio(id_medio: int, medio: medios_schema.MedioCreate, db: Session = Depends(get_db)):
    db_medio = medios_crud.get_medio_by_id(db, id_medio=id_medio)
    if db_medio is None:
        raise HTTPException(status_code=404, detail='Medio no encontrado')
    return medios_crud.update_medio(db=db, id_medio=id_medio, medio=medio)

# Ruta para eliminar un medio
@router.delete('/delete/{id_medio}', response_model=medios_schema.Medio)
def delete_medio(id_medio: int, db: Session = Depends(get_db)):
    db_medio = medios_crud.get_medio_by_id(db, id_medio=id_medio)
    if db_medio is None:
        raise HTTPException(status_code=404, detail='Medio no encontrada')
    return pruebas_crud.delete_prueba(db=db, id_prueba=id_prueba)
