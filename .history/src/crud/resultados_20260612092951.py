...
def update_resultado(db: Session, id_resultado: int, resultado: ResultadoCreate):
    db_resultado = db.query(Resultado_prueba_micro).filter(Resultado_cuestionario.id_resultado_cues == id_resultado_cues).first()
    if not db_resultado:
        return None
