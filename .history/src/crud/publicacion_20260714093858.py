...
db_publicacion = publicacion_crud.get_publicacion_by_id(db, id_publicacion=id_publicacion)
    if db_publicacion is None:
        raise HTTPException(status_code=404, detail="Esa publicación no existe")
    ...
    if db_publicacion_existente:
        raise HTTPException(status_code=400, detail="Ya existe otra publicación con ese título y contenido")
