...
def update_prueba(db: Session, id_prueba: int, prueba: PruebaCreate):
    db_prueba = db.query(Pruebas_bioquimicas).filter_by(id_prueba=id_prueba).first()
    if not db_prueba:
        return None
    ...
    db.query(Opcion).filter(Opcion.id_prueba == id_prueba).delete()
    ...
    db.commit()
    db.refresh(db_prueba)
    _ = db_prueba.opciones  # fuerza carga
    return db_prueba
