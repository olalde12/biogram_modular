...
def update_medio(db: Session, id_medio: int, medio: MedioCreate):
    ...
    if not db_medio:
        return None
    db_medio.nombre = prueba.nombre
    db_medio.tipo_resultado = prueba.tipo_resultado
    db_medio.fundamento = prueba.fundamento
    db_medio.tiempo_reaccion = prueba.tiempo_reaccion
    db_medio.descripcion = prueba.descripcion
