...
createUser.addEventListener("submit", async (event) => {
    ...
    if (!expresiones.email.test(email)) {
        document.getElementById("error_email").innerText = "El correo no es válido o ya existe";
        document.getElementById("error_email").style = 'display: block';
    } else {
        document.getElementById("error_email").style = 'display: none';
    }

    const userData = {nombre: nombre, apellido: apellido, correo: correo, contrasena: contrasena};
    ...
    try {
        ...
        // Error de validación (correo ya existe o datos inválidos)
        const errorData = await response.json();
            document.getElementById("error_correo").innerText = errorData.detail || "Ese correo ya existe";
